...
os.system("pactl load-module module-loopback latency_msec=1 channel_map=left")
...
def process_audio_data(audiodata):
    ...
    ch2 = butter_highpass_filter(ch1, 10, samplerate)
...
filename = datetime.datetime.strftime(t_0,
                                          '%Y_%m_%d_%H_%M_%S_pyaudio')
# Write the processed audio data to a wav file.
write(filename+'.wav', int(samplerate/downsample), processed)
